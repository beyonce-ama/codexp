...
public function generateShortAI(Request $request)
{
    $data = $request->validate([
        'language'   => 'required|in:python,java,cpp',
        'difficulty' => 'required|in:easy,medium,hard',
    ...
    try {
        ...
        // Return only the fields we want the UI to preview (ignore hint on save)
        $out = [
            'title'       => (string)($challenge['title'] ?? ''),
            'description' => (string)($challenge['description'] ?? ''),
            'buggy_code'  => (string)($challenge['buggy_code'] ?? ''),
            'fixed_code'  => (string)($challenge['fixed_code'] ?? ''),
            // hint may be returned, but frontend will not present it if you prefer
