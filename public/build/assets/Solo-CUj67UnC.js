...
`).replace(/\s+/g," ").replace(/\s*;\s*/g,";").replace(/\s*\{\s*/g,"{").replace(/\s*\}\s*/g,"}").replace(/\s*\(\s*/g,"(").replace(/\s*\)\s*/g,")").replace(/\s*==\s*/g,"==").replace(/\s*=\s*/g,"=").trim(),p=o(s),l=o(i);if(p===l)return console.log("Code validation: PASS - Exact match with database solution (100%)"),!0;const u=he(p,l),b=Math.round(u*100);console.log(`Code validation: FAIL - Only ${b}% similarity with database solution (requires 100%)`);const N=u>=1;return console.log(`Code validation: ${N?"PASS":"FAIL"} (${b}% similarity, requires 100%)`),N},Xe=async()=>{var t;if(!r||!h.trim()){d.play("failure"),f.fire("Error","Please write some code before submitting.","error");return}try{if(O(!0),d.play("typing"),!r.fixed_code){d.play("failure"),f.fire("Error","This challenge does not have a solution stored in the database.","error"),O(!1);return}const a=ze(h,r),s=he(h.trim().replace(/\s+/g," "),r.fixed_code.trim().replace(/\s+/g," "));W({isCorrect:a,similarity:s}),G(!0);const i={challenge_id:r.id,language:r.language,mode:r.mode,time_spent_sec:v,is_correct:a,code_submitted:h,judge_feedback:a?"Perfect! Your solution is an exact match with the expected answer from database (100% match).":`Incorrect. Your solution has ${Math.round(s*100)}% similarity with the expected answer. You need 100% match to pass. Try again or view the correct answer.`},o=await w.post("/api/solo/attempts",i);if(o.success){const p=((t=o.data)==null?void 0:t.xp_earned)||(a?r.reward_xp:0);if(a){await w.post("/api/solo/mark-taken",{challenge_id:r.id,language:r.language,difficulty:r.difficulty,mode:r.mode,status:"completed",time_spent_sec:v,code_submitted:h,earned_xp:p??0});const l=(c==null?void 0:c.total_xp)||0,m=l+p,u=ee(l),b=ee(m),N=b>u;c&&!c.completed_challenge_ids.includes(r.id)&&U(X=>X?{...X,completed_challenge_ids:[...X.completed_challenge_ids,r.id],successful_attempts:X.successful_attempts+1,total_xp:m,current_level:b,xp_to_next_level:te(m)}:null),d.play("success"),K(!0),V(!0),Z(!0),pe(!0),setTimeout(()=>pe(!1),2e3);const k=window.innerWidth/2,_=window.innerHeight/2;ge(k,_,"success"),N&&setTimeout(()=>{d.play("levelup"),Q(!0),ge(k,_-100,"levelup")},1e3),setTimeout(()=>{d.play("victory")},500);const{isConfirmed:ae}=await f.fire({title:"PERFECT SOLUTION!",html:`
  ...
  `,showConfirmButton:!0,confirmButtonText:"Continue Coding!",confirmButtonColor:"#10B981",background:"linear-gradient(135deg, #1e3a8a 0%, #312e81 100%)",color:"#fff",allowOutsideClick:!1,allowEscapeKey:!1});ae&&(K(!1),V(!1),Q(!1),Z(!1),D(!1),Y(null),E(""),H(null),L(0),G(!1),W(null),J(!1),await Promise.all([I(),$(),M()])),setTimeout(()=>{K(!1),V(!1),Q(!1),Z(!1)},7e3)}else d.play("failure"),me(!0),setTimeout(()=>me(!1),600),await f.fire({title:"Almost There!",html:`
                                              <div class="text-center">
    ...
    `,showCancelButton:!0,confirmButtonText:"Show Answer",cancelButtonText:"Cancel",background:"#0f172a",color:"#f8fafc",confirmButtonColor:"#ef4444",cancelButtonColor:"#6b7280",width:600,customClass:{popup:"rounded-xl border border-gray-700/50",htmlContainer:"p-0",confirmButton:"px-5 py-2 rounded-lg font-semibold",cancelButton:"px-5 py-2 rounded-lg font-semibold"}})).isConfirmed||(d.play("click"),await f.fire({title:"",html:`
      ...
      <div class="bg-slate-900 border border-slate-700 rounded-xl p-5 shadow-lg text-left">
        <div class="mb-3 flex items-center justify-between border-b border-slate-700 pb-2">
          <span class="text-yellow-400 font-semibold">🏆 ${(r==null?void 0:r.title)??"Correct Answer"}</span>
    ...
    `,width:700,background:"#0f172a",color:"#f8fafc",confirmButtonText:"Close",confirmButtonColor:"#10B981",customClass:{popup:"rounded-xl border border-slate-700 shadow-2xl backdrop-blur-sm",htmlContainer:"p-0",confirmButton:"px-6 py-2 rounded-lg font-semibold"},didOpen:()=>{const a=document.getElementById("solo-surrender-solution");a&&(a.textContent=r.fixed_code||"")}}),await Ue("abandoned"))},Ye=()=>{r!=null&&r.fixed_code&&(d.play("click"),J(!0),Ge("Correct Answer",r.fixed_code),f.fire({title:"Correct Answer",html:`
